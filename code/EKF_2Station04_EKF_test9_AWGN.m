...
for s = 1:nStation
    ...
    idx_valid = isfinite(doppler_error(:,s));
    doppler_rmse = sqrt(mean(doppler_error(idx_valid,s).^2));
    ...
    ylabel('Error (Hz)');
    xlabel('Time UTC');
    title(sprintf('%s Doppler Error (RMSE = %.3f Hz)', ...
...
for s = 1:nStation
    ...
    idx_az_rmse = isfinite(az_err);
    idx_el_rmse = isfinite(el_err);
    ...
    grid on; yline(0,'--');
    ylabel('Az Error (deg)');
    title(sprintf('%s Azimuth Error (RMSE = %.3f deg)', ...
    ...
    ylabel('El Error (deg)');
    xlabel('Time UTC');
    title(sprintf('%s Elevation Error (RMSE = %.3f deg)', ...

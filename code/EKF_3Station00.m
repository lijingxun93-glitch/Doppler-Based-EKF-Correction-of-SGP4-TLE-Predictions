%% ========================================================================
%  Multi-ground-station Doppler EKF for SGP4/TLE correction
%  State:
%  x = [dM, dRAAN, dARGP, dINC, de, dn, dBstar]^T
%
%  Measurement:
%  z = [Doppler_station_1; ...; Doppler_station_N]
%
%  If a station is below horizon or has no matched measurement, only the
%  available stations are used in each EKF update.
%% ========================================================================
...
%% ----------------------------- Ground stations ----------------------------
% 直接在这里增加/删除观测站即可。
% 注意：每个站都需要对应一份 measured_data，或者用下面的 pseudo measured data 测试。
gs = struct('lat_deg',{},'lon_deg',{},'alt_m',{});

% Station 1: Annaba
gs(1).lat_deg = 36.8333;
gs(1).lon_deg = 7.8167;
gs(1).alt_m   = 4;

% Station 2: Aoulef
gs(2).lat_deg = 26.9670;
gs(2).lon_deg = 1.0830;
gs(2).alt_m   = 309;

% Station 3: Bechar
gs(3).lat_deg = 31.6170;
gs(3).lon_deg = -2.233;
gs(3).alt_m   = 773;
...
% 
% measFile = strings(nStation,1);
% measFile(1) = "C:\Users\22743\Desktop\Master project\satellite data\20251101\station1.csv";
% measFile(2) = "C:\Users\22743\Desktop\Master project\satellite data\20251101\station2.csv";
% measFile(3) = "C:\Users\22743\Desktop\Master project\satellite data\20251101\station3.csv";
% % 如果有更多站，继续设置 measFile(4), measFile(5), ...
%
% measured_data = cell(nStation,1);
% t_real        = cell(nStation,1);
% idx_real      = cell(nStation,1);
%
% % 每个站录像/数据的 UTC 起始时间
% measStartTime = repmat(datetime(2026,5,15,14,5,24,'TimeZone','UTC'), nStation, 1);
% % 如果某个站起始时间不同，单独覆盖：
% % measStartTime(2) = datetime(2026,5,15,14,5,30,'TimeZone','UTC');
% % measStartTime(3) = datetime(2026,5,15,14,5,35,'TimeZone','UTC');
%
% for s = 1:nStation
...
% measStartTime = datetime(2026,5,27,5,53,55,'TimeZone','UTC');
measStartTime(1) = datetime(2026,5,27,5,53,55,'TimeZone','UTC');
measStartTime(2) = measStartTime(1);
measStartTime(3) = measStartTime(1);
% 如果不同站的录像/数据起始时间不同，单独覆盖：
% measStartTime(2) = datetime(2026,5,15,14,5,30,'TimeZone','UTC');
% measStartTime(3) = datetime(2026,5,15,14,5,35,'TimeZone','UTC');
...
% satdata_err.xmo    = satdata.xmo    + deg2rad(1);   % Mean anomaly: orbital phase
% satdata_err.xnodeo = satdata.xnodeo + deg2rad(0.5);   % RAAN: orbit plane orientation
% satdata_err.omegao = satdata.omegao + deg2rad(0.5);   % Argument of perigee: ellipse orientation in orbital plane
% satdata_err.xincl  = satdata.xincl  + deg2rad(0.5);   % Inclination: orbit tilt
% satdata_err.eo     = satdata.eo     + 5e-4;           % Eccentricity: orbit shape
% satdata_err.xno    = satdata.xno    * (1 + 1e-4);     % Mean motion: average angular rate
% satdata_err.bstar  = satdata.bstar  * (1 + 1e-8);      % B*: atmospheric drag parameter
...
% 用所有观测站数据中最短的结束时间，避免索引越界
endTime = min(cellfun(@(tt) tt(end), t_real));
...
% 每个站的 Doppler measurement noise standard deviation [Hz]
% 如果某个站接收机更差，例如第三站 5 Hz，可设成 [1; 1; 5]
R_doppler_std_Hz = 20*ones(nStation,1);
R_full = diag(R_doppler_std_Hz.^2);      % Hz^2
%% ============================= EKF main loop ===============================
for j = 1:N
...
%% ============================= Plot results ================================
figure(1);
set(gcf,'Name','Multi-station Doppler EKF','Color','w');
tl = tiledlayout(nStation, 1, 'TileSpacing','compact', 'Padding','compact');
...
for s = 1:nStation
    ...
    yyaxis right;
    plot(t_EKF(idx_valid), doppler_error(idx_valid,s), '--', 'LineWidth', 1.0);
    ylabel('Error (Hz)');

    yyaxis left;
    ylabel('Doppler (Hz)');
    title(sprintf('Station %d Doppler and residual',s));
    legend('Measured','SGP4 + EKF','Residual','Location','best');
end

figure(2);
nexttile;
plot(t_EKF, double(used_station_flag), 'LineWidth', 1.1);
grid on;
ylim([-0.1 1.1]);
ylabel('Used in EKF');
title('Station usage');
legend(arrayfun(@(s) sprintf('Station %d',s), 1:nStation, 'UniformOutput', false), ...
...
figure(3);
set(gcf,'Name','Az El comparison','Color','w');
tl2 = tiledlayout(nStation, 2, 'TileSpacing','compact', 'Padding','compact');

clear;
measured_data_1 = readtable('C:\Users\22743\Desktop\Master project\satellite data\HST 20260704\epoch 20260523 SGP4_Az_El_Doppler_Visible.csv');
measured_data_2 = readtable('C:\Users\22743\Desktop\Master project\satellite data\HST 20260704\epoch 20260702 SGP4_Az_El_Doppler_Visible.csv');
% AWGN
sigma_D = 10;
rng(1);
...
legend('TLE epoch at 2026-05-23', 'TLE epoch at 2026-07-16');
hold off;
...
startTime = datetime(2026,7,4,7,19,53);
...
legend('TLE epoch at 2026-05-23', 'TLE epoch at 2026-07-16', 'Location','best'); 
title('Doppler shifts');
grid on; 
ylabel('Doppler (Hz)');
hold off;
...
% Display every datetime x-axis using English month names.

    axes_list = findall(fig, 'Type', 'axes');

    for ii = 1:numel(axes_list)
        tick_times = xticks(axes_list(ii));

        if isdatetime(tick_times)
            tick_labels = string( ...
                tick_times, 'dd-MMM-yyyy HH:mm', 'en_US');
            xticklabels(axes_list(ii), tick_labels);
        end
    end
end

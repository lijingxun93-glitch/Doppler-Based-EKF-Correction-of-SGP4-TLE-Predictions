...
startTime  = datetime(2026,6,22,5,43,0,"TimeZone","UTC");
...
if ~isempty(t_visible)
    ...
    subplot(3,1,1);
    plot(t_visible, az_visible, 'LineWidth', 1.2);
    grid on;
    ylabel("Azimuth (deg)");
    title("Visible Pass Only: El > 0");

    subplot(3,1,2);
    plot(t_visible, el_visible, 'LineWidth', 1.2);
    grid on;
    ylabel("Elevation (deg)");

    subplot(3,1,3);
    plot(t_visible, fd_visible, 'LineWidth', 1.2);
    grid on;
    ylabel("Doppler (Hz)");
    xlabel("UTC Time");
end
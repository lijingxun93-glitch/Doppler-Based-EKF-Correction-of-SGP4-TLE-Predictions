...
% with full paths.
measFile(1) = "C:\Users\22743\Desktop\Master project\satellite data\20251101\SGP4_Az_El_Doppler_All_Tehran.csv";
measFile(2) = "C:\Users\22743\Desktop\Master project\satellite data\20251101\SGP4_Az_El_Doppler_All_Tabriz.csv";
...
for s = 1:nStation
    ...
    measured_data{s}.Doppler_Hz    = Tcsv.("Doppler_Hz");
...
% satdata_err.xmo    = satdata.xmo    + deg2rad(1);
% satdata_err.xnodeo = satdata.xnodeo + deg2rad(0.5);
% satdata_err.omegao = satdata.omegao + deg2rad(0.5);
% satdata_err.xincl  = satdata.xincl  + deg2rad(0.2);
% satdata_err.eo     = satdata.eo     + 5e-4;
% satdata_err.xno    = satdata.xno    * (1 + 1e-4);
% satdata_err.bstar  = satdata.bstar  * (1 + 1e-8);
...
%% ----------------------------- Simulation time ----------------------------
dt = 1;                                      % second
duration_sec = floor(seconds(endTime - startTime));
N = duration_sec + 1;
...
for s = 1:nStation
    ...
    legend('CSV Doppler','SGP4 + EKF','Location','best');
end
...
for s = 1:nStation
    ...
    ylabel('Error (Hz)');
    xlabel('Time UTC');
    title(sprintf('%s Doppler Error', stationName(s)));
end
...
for s = 1:nStation
    ...
    legend('CSV','SGP4 + EKF','Location','best');
    ...
    grid on; yline(0,'--');
    ylabel('Az Error (deg)');
    title(sprintf('%s Azimuth Error', stationName(s)));
    ...
    ylabel('El Error (deg)');
    xlabel('Time UTC');
    title(sprintf('%s Elevation Error', stationName(s)));
end

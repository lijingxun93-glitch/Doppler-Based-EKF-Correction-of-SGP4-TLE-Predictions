...
% P与误差同数量级
% P = diag([1 1 1 1e-3 1e-3 1e-3].^2);
P = diag([50 50 50 0.05 0.05 0.05].^2);

% process noise, first guess
Q = diag([1e-2 1e-2 1e-2 1e-3 1e-3 1e-3].^2);

% measurement noise: Doppler only
R = 1^2;   % Hz^2, first guess
...
function H = numerical_H_doppler(x, recef, vecef, rsite_ecef, vsite_ecef, gs, fc, c)
    ...
    dx_step = [1e-3; 1e-3; 1e-3; 1e-6; 1e-6; 1e-6];
    % dr: km
    % dv: km/s

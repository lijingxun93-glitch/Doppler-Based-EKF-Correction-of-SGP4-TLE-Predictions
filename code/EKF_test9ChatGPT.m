...
%% ===================== Measured data =====================
measured_data = readtable('C:\Users\22743\Desktop\Master project\satellite data\NOAA15_EKF9\SGP4_Az_El_Doppler_Visible.csv');
...
measured_data.Doppler_Hz = measured_data.Doppler_Hz + sigma_D * randn(size(measured_data.Doppler_Hz));
...
%satdata_err.xnodeo = satdata.xnodeo + deg2rad(2);    % RAAN
%% ===================== Read EOP =====================
...
% RMSE is calculated only from finite error samples above the horizon.
idx_valid_doppler   = isfinite(doppler_error);
idx_valid_azimuth   = isfinite(azimuth_error);
idx_valid_elevation = isfinite(elevation_error);

rmse_doppler   = sqrt(mean(doppler_error(idx_valid_doppler).^2));
rmse_azimuth   = sqrt(mean(azimuth_error(idx_valid_azimuth).^2));
rmse_elevation = sqrt(mean(elevation_error(idx_valid_elevation).^2));
...
subplot(3,2,2);
plot(t_EKF(idx_valid_doppler), doppler_error(idx_valid_doppler), 'LineWidth', 1.2);
grid on;
xlabel('Time (UTC)');
ylabel('Doppler error (Hz)');
title(sprintf('Doppler Error above horizon (RMSE = %.3f Hz)', rmse_doppler));
yline(0,'--');
...
subplot(3,2,4);
plot(t_EKF(idx_valid_azimuth), azimuth_error(idx_valid_azimuth), 'LineWidth', 1.2);
grid on;
xlabel('Time (UTC)');
ylabel('Azimuth error (deg)');
title(sprintf('Azimuth Error above horizon (RMSE = %.3f deg)', rmse_azimuth));
yline(0,'--');
...
subplot(3,2,6);
plot(t_EKF(idx_valid_elevation), elevation_error(idx_valid_elevation), 'LineWidth', 1.2);
grid on;
xlabel('Time (UTC)');
ylabel('Elevation error (deg)');
title(sprintf('Elevation Error above horizon (RMSE = %.3f deg)', rmse_elevation));
yline(0,'--');
